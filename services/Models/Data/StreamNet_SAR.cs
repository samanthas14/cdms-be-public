...
{
    public static class StreamNet_SARExtensions
    {
        //Extension method to give ServicesContext this property.
        public static DbSet<StreamNet_SAR_Header> StreamNet_SAR_Header(this ServicesContext ctx)
        {
            return ctx.GetDbSet("StreamNet_SAR_Header").Cast<StreamNet_SAR_Header>();
        }

        public static DbSet<StreamNet_SAR_Detail> StreamNet_SAR_Detail(this ServicesContext ctx)
        {
            return ctx.GetDbSet("StreamNet_SAR_Detail").Cast<StreamNet_SAR_Detail>();
        }
    }
}
...
{
    public class StreamNet_SAR : DatasetData
    {
        public Dataset Dataset { get; set; }
        public StreamNet_SAR_Header Header { get; set; }
        public List<StreamNet_SAR_Detail> Details { get; set; }

        public StreamNet_SAR()
        {
            Details = new List<StreamNet_SAR_Detail>();
        }


        // load an existing one
        public StreamNet_SAR(int ActivityId)
        {
            var ndb = ServicesContext.Current;
            Details = new List<StreamNet_SAR_Detail>();

            //select header by activityid (taking effdt into account)
            var headers_q = from h in ndb.StreamNet_SAR_Header()
                            where h.ActivityId == ActivityId
                            join h2 in
                                (
                                    from hh in ndb.StreamNet_SAR_Header()
            ...
            var details_q = from h in ndb.StreamNet_SAR_Detail()
                            where h.ActivityId == ActivityId
                            where h.RowStatusId == DataDetail.ROWSTATUS_ACTIVE
                            join h2 in
                                (
                                    from hh in ndb.StreamNet_SAR_Detail()
                                    where hh.EffDt <= DateTime.Now
                                    where hh.ActivityId == ActivityId
