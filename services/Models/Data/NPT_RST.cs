...
{
    public static class NPT_RSTExtensions
    {
        //Extension method to give ServicesContext this property.
        public static DbSet<NPT_RST_Header> NPT_RST_Header(this ServicesContext ctx)
        {
            return ctx.GetDbSet("NPT_RST_Header").Cast<NPT_RST_Header>();
        }

        public static DbSet<NPT_RST_Detail> NPT_RST_Detail(this ServicesContext ctx)
        {
            return ctx.GetDbSet("NPT_RST_Detail").Cast<NPT_RST_Detail>();
        }
    }
}
...
{
    public class NPT_RST : DatasetData
    {
        public Dataset Dataset { get; set; }
        public NPT_RST_Header Header { get; set; }
        public List<NPT_RST_Detail> Details { get; set; }

        public NPT_RST()
        {
            Details = new List<NPT_RST_Detail>();
        }


        // load an existing one
        public NPT_RST(int ActivityId)
        {
            var ndb = ServicesContext.Current;
            Details = new List<NPT_RST_Detail>();

            //select header by activityid (taking effdt into account)
            var headers_q = from h in ndb.NPT_RST_Header()
                            where h.ActivityId == ActivityId
                            join h2 in
                                (
                                    from hh in ndb.NPT_RST_Header()
            ...
            var details_q = from h in ndb.NPT_RST_Detail()
                            where h.ActivityId == ActivityId
                            where h.RowStatusId == DataDetail.ROWSTATUS_ACTIVE
                            join h2 in
                                (
                                    from hh in ndb.NPT_RST_Detail()
                                    where hh.EffDt <= DateTime.Now
                                    where hh.ActivityId == ActivityId
