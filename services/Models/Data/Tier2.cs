...
{
    public static class Tier2Extensions
    {
        //Extension method to give ServicesContext this property.
        public static DbSet<Tier2_Header> Tier2_Header(this ServicesContext ctx)
        {
            return ctx.GetDbSet("Tier2_Header").Cast<Tier2_Header>();
        }

        public static DbSet<Tier2_Detail> Tier2_Detail(this ServicesContext ctx)
        {
            return ctx.GetDbSet("Tier2_Detail").Cast<Tier2_Detail>();
        }
    }
}
namespace services.Models.Data
{
    public class Tier2 : DatasetData
    {
        private Boolean debugMode = true;
        private static Logger logger = LogManager.GetCurrentClassLogger();

        public Dataset Dataset { get; set; }
        public Tier2_Header Header { get; set; }
        public List<Tier2_Detail> Details { get; set; }

        public Tier2()
        {
            Details = new List<Tier2_Detail>();
        }


        // load an existing one
        public Tier2(int ActivityId)
        {
            ...
            var headers_q = from h in db.Tier2_Header()
                            where h.ActivityId == ActivityId
                            join h2 in
                                (
                                    from hh in db.Tier2_Header()
            ...
            var details_q = from h in db.Tier2_Detail()
                            where h.ActivityId == ActivityId
                            where h.RowStatusId == DataDetail.ROWSTATUS_ACTIVE
                            join h2 in
                                (
                                    from hh in db.Tier2_Detail()
                                    where hh.EffDt <= DateTime.Now
                                    where hh.ActivityId == ActivityId
